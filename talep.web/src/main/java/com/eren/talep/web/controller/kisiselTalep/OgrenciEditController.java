...
@Controller
public class OgrenciEditController {
    ...
    @RequestMapping(method = RequestMethod.GET, value = "/talep/ogrenciEdit/{talepid}")
    public String viewOgrenciEdit(@PathVariable Integer talepid, Model map, HttpSession session) {
        ...
        if (talepid != 0) {//ogrenci Edit kısmı için 
        ...
        } else { // yeniogrenci kısmı için özel olan yerler
//            editYapilanIslem.setGenelTalep(new GenelTalep());
//            editYapilanIslem.getGenelTalep().setTalep(new Talep());
            String ogrenciDivHeight = "yeniOgrenci";
            map.addAttribute("ogrenciDivHeight", ogrenciDivHeight);
        }
        ...
        if (talepid == 0) {
            return "kisiselTalep/ogrenciYeni";
        } else {
            return "kisiselTalep/ogrenciEdit";
        }
    }

    @RequestMapping(method = RequestMethod.POST, value = "/ogrenciEdit/talepEklendi/{secim}")
    public String saveOgrenciYeni(@PathVariable String secim, @RequestParam String islemTarih, @RequestParam String talepTarih, Model model, HttpSession session, @ModelAttribute YapilaniSlem editYapilanIslem, SessionStatus status) throws ParseException {
        User user;
        ...
        if (secim.equals("yeni")) {//Yeni Talep Ekleneceği Zaman
            ...
            editYapilanIslem.getGenelTalep().setTalepTipi("ogrenci");
    ...
    @RequestMapping(method = RequestMethod.GET, value = "/ogrenciEdit/pagedList/{keyword}")
    public String viewOgrenciPagedList(@PathVariable String keyword, Model map, HttpSession session) {
