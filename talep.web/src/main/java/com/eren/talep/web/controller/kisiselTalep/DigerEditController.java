...
@Controller
public class DigerEditController {
     @Autowired
    ...
    public void setServiceFactory(ServiceFactory serviceFactory) {
     ...
     @RequestMapping(method = RequestMethod.GET, value = "/talep/digerEdit/{talepid}")
    public String viewDigerEdit(@PathVariable Integer talepid, Model map, HttpSession session) {
        ...
        if (talepid != 0) {//diger Edit kısmı için 
        ...
        } else { // yenidiger kısmı için özel olan yerler
//            editYapilanIslem.setGenelTalep(new GenelTalep());
//            editYapilanIslem.getGenelTalep().setTalep(new Talep());
            String digerDivHeight = "yeniDiger";
            map.addAttribute("digerDivHeight", digerDivHeight);
        }
        ...
        if (talepid == 0) {
            return "kisiselTalep/digerYeni";
        } else {
            return "kisiselTalep/digerEdit";
        }
    }

    @RequestMapping(method = RequestMethod.POST, value = "/digerEdit/talepEklendi/{secim}")
    public String saveDigerYeni(@PathVariable String secim, @RequestParam String islemTarih, @RequestParam String talepTarih, Model model, HttpSession session, @ModelAttribute YapilaniSlem editYapilanIslem, SessionStatus status) throws ParseException {
        User user;
        ...
        if (secim.equals("yeni")) {//Yeni Talep Ekleneceği Zaman
            ...
            editYapilanIslem.getGenelTalep().setTalepTipi("diger");
    ...
    @RequestMapping(method = RequestMethod.GET, value = "/digerEdit/pagedList/{keyword}")
    public String viewDigerPagedList(@PathVariable String keyword, Model map, HttpSession session) {
