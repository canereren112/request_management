...
public class IsEditController {
    ...
    @RequestMapping(method = RequestMethod.GET, value = "/talep/isEdit/{talepid}")
    public String viewIsEdit(@PathVariable Integer talepid, Model map, HttpSession session) {
        ...
        if (talepid != 0) {//is Edit kısmı için 
        ...
        } else { // yeniis kısmı için özel olan yerler
//            editYapilanIslem.setGenelTalep(new GenelTalep());
//            editYapilanIslem.getGenelTalep().setTalep(new Talep());
            String isDivHeight = "yeniIs";
            map.addAttribute("isDivHeight", isDivHeight);
        }
        ...
        if (talepid == 0) {
            return "kisiselTalep/isYeni";
        } else {
            return "kisiselTalep/isEdit";
        }
    }

    @RequestMapping(method = RequestMethod.POST, value = "/isEdit/talepEklendi/{secim}")
    public String saveIsYeni(@PathVariable String secim, @RequestParam String islemTarih, @RequestParam String talepTarih, Model model, HttpSession session, @ModelAttribute YapilaniSlem editYapilanIslem, SessionStatus status) throws ParseException {
        User user;
        ...
        if (secim.equals("yeni")) {//Yeni Talep Ekleneceği Zaman
            ...
            editYapilanIslem.getGenelTalep().setTalepTipi("is");
    ...
    @RequestMapping(method = RequestMethod.GET, value = "/isEdit/pagedList/{keyword}")
    public String viewIsPagedList(@PathVariable String keyword, Model map, HttpSession session) {
