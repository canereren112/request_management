...
@Controller
public class YardimEditController {
     @Autowired
    ...
    @RequestMapping(method = RequestMethod.GET, value = "/talep/yardimEdit/{talepid}")
    public String viewYardimEdit(@PathVariable Integer talepid, Model map, HttpSession session) {
        ...
        if (talepid != 0) {//yardim Edit kısmı için 
        ...
        } else { // yeniyardim kısmı için özel olan yerler
//            editYapilanIslem.setGenelTalep(new GenelTalep());
//            editYapilanIslem.getGenelTalep().setTalep(new Talep());
            String yardimDivHeight = "yeniYardim";
            map.addAttribute("yardimDivHeight", yardimDivHeight);
        }
        ...
        if (talepid == 0) {
            return "kisiselTalep/yardimYeni";
        } else {
            return "kisiselTalep/yardimEdit";
        }
    }

    @RequestMapping(method = RequestMethod.POST, value = "/yardimEdit/talepEklendi/{secim}")
    public String saveYardimYeni(@PathVariable String secim, @RequestParam String islemTarih, @RequestParam String talepTarih, Model model, HttpSession session, @ModelAttribute YapilaniSlem editYapilanIslem, SessionStatus status) throws ParseException {
        User user;
        ...
        if (secim.equals("yeni")) {//Yeni Talep Ekleneceği Zaman
            ...
            editYapilanIslem.getGenelTalep().setTalepTipi("yardim");
    ...
    @RequestMapping(method = RequestMethod.GET, value = "/yardimEdit/pagedList/{keyword}")
    public String viewYardimPagedList(@PathVariable String keyword, Model map, HttpSession session) {
